...
const resourceName = "medications"
...
func New(db *store.DB, limits Limits, dataDir string) *Server {
	...
	s.mux.HandleFunc("GET /api/tier", func(w http.ResponseWriter, r *http.Request) {
		wj(w, 200, map[string]any{
			"tier":        s.limits.Tier,
			"upgrade_url": "https://stockyard.dev/apothecary/",
		})
	})
...
func (s *Server) loadPersonalConfig() {
	...
	if err := json.Unmarshal(data, &cfg); err != nil {
		log.Printf("apothecary: warning: could not parse config.json: %v", err)
		return
	}
	s.pCfg = cfg
	log.Printf("apothecary: loaded personalization from %s", path)
}
...
func (s *Server) list(w http.ResponseWriter, r *http.Request) {
	...
	if q != "" || len(filters) > 0 {
		wj(w, 200, map[string]any{"medications": oe(s.db.Search(q, filters))})
		return
	}
...
func (s *Server) create(w http.ResponseWriter, r *http.Request) {
	if s.limits.MaxItems > 0 && s.db.Count() >= s.limits.MaxItems {
		we(w, 402, "Free tier limit reached. Upgrade at https://stockyard.dev/apothecary/")
		return
	}
	var e store.Medication
...
func (s *Server) update(w http.ResponseWriter, r *http.Request) {
	...
	if patch.Dosage == "" {
		patch.Dosage = existing.Dosage
	}
	if patch.Frequency == "" {
		patch.Frequency = existing.Frequency
	}
	if patch.Prescriber == "" {
		patch.Prescriber = existing.Prescriber
	}
	if patch.Pharmacy == "" {
		patch.Pharmacy = existing.Pharmacy
	}
	if patch.RefillDate == "" {
		patch.RefillDate = existing.RefillDate
	}
	if patch.Notes == "" {
